...
def find_data_file(stl_dir, model_name, layer_thk):
    ...
    stl_name = f"{model_name}.stl"
    stl_path = os.path.join(stl_dir, stl_name)
    if not os.path.exists(stl_path): stl_path = os.path.join(stl_dir, f"{model_name}.STL")

    if os.path.exists(stl_path):
        print(f"正在生成临时切片: {slc_name} ...")
        model = StlModel()
        model.readStlFile(stl_path)
        layers = IntersectStl_sweep(model, layer_thk).layers
        for l in layers:
            if l.segments: l.contours = linkSegs_brutal(l.segments)
        writeSlcFile(layers, slc_path)
        return slc_path
    return None
...
def show_clean_contours():
    ...
    if layer.contours:
        # 现在 genCpPath 函数已经定义了，不会报错
        paths = genCpPath(layer.contours, INTERVAL, SHELL_THK)
        print(f"    - 生成路径数: {len(paths)} (已过滤噪点)")
    else:
        paths = []

    print("[4] 渲染视图...")
    va = VtkAdaptor(bgClr=(1.0, 1.0, 1.0))

    # 1. 原始轮廓 (黑色加粗)
    for c in layer.contours:
        p = c.clone()
        for pt in p.points: pt.z = 0
